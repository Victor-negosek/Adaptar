...
api = Namespace('tool', description="This is the API to handle with the tools")

@api.route('/<productName>/<stepName>')
class Tool(Resource):
    @api.doc('This is the API to get the tool to a specific product and step')   
    def get(self, productName, stepName):
        header = current_app.config.get("BASEHEADER")
        header["Authorization"] = request.headers.get('Authorization')
        query = {"query": "SELECT value FROM DIGITALTWINS MATCH (step)-[r:use]->(value)<-[use]-(product) WHERE step.$dtId = '{}' AND product.$dtId = '{}'".format( stepName, productName)}
